...
```{r}
poses <- read_csv("frame_data/pose_collated_1000.csv")

```
Looks like some of the 10k sample frames are from Bing. 

```{r}
poses$video_hashid[!(poses$video_hashid %in% recordings$superseded_gcp_name_feb25)]
...
poses_joined <- inner_join(poses, d, by = c("video_hashid" = "superseded_gcp_name_feb25"))
```
