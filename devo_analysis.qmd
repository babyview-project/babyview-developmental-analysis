...
Calculate raw counts (not normalizing right now)
```{r}
...
Plot top 50 objects in the 1k frame detections
```{r}
ggplot(data=object_freq %>% slice_max(n=50, order_by=count), aes(x=fct_reorder(class_name, count, .desc=FALSE), y=count)) +
...
```{r}
ggplot(data=object_freq_with_aoa, aes(x=count, y=AoA)) +
  geom_point(alpha=.4) +
  xlab('Count of objects')
  ylab('AoA') +
  theme_few(base_size=6)  +
  geom_smooth(method='lm')
```
