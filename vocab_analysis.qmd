...
```{r}
cdi_eng_wg <- read_csv(here("cdi_data_march_2025", "babyview-english-wg_items.csv")) |> 
  mutate(language = "English (American)", form = "WG")
cdi_eng_ws <- read_csv(here("cdi_data_march_2025", "babyview-english-ws_items.csv")) |> 
  mutate(language = "English (American)", form = "WS")
cdi_spa_wg <- read_csv(here("cdi_data_march_2025", "babyview-spanish-wg_items.csv")) |> 
  mutate(language = "Spanish (Mexican)", form = "WG")
cdi_spa_ws <- read_csv(here("cdi_data_march_2025", "babyview-spanish-ws_items.csv")) |> 
...
cdi_items <- read_csv(here("cdi_items.csv"))

cdi_all <- cdi_all |> 
  left_join(cdi_items, by = join_by(language, form, item == webcdi_item))

write_csv(cdi_all, here("cdi_all.csv"))
```
